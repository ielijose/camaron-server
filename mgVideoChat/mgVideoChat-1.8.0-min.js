...
(function( $, window, document, undefined ){
    ...
    mgVideoChat.prototype.updateLayout = function(){
        ...
        if(mainVisible){
            $mainContent.removeClass().addClass('col-sm-8 col-xs-12 open');
            $sideMenu.removeClass().addClass('col-sm-4 col-xs-6 sidebar-offcanvas');
            $('#offcanvasButton').show();
        }
        else{
            $mainContent.removeClass().addClass('col-sm-0');
            $sideMenu.removeClass().addClass('col-sm-8 sidebar-offcanvas');
        }
    ...
    mgVideoChat.prototype.onVideoOpen = function(connectionId){
        ...
        if(connectionId){
            this.$callPanel.find('.panel-title').text(this._('Llamada con {username}',['{username}'],[rtc.connections[connectionId]['data']['userData']['name']]));
        }
        this.$callPanel.show();
        this.updateLayout();
    ...
    mgVideoChat.prototype.localVideoOpen = function(stream, connectionId){
        if(stream){
            this.$elem.find("#localVideo").attr("src",window.URL.createObjectURL(stream));
    ...
    mgVideoChat.prototype.remoteVideoOpen = function(stream, connectionId){
        if(stream){
            this.$elem.find("#remoteVideo").attr("src",window.URL.createObjectURL(stream));
    ...
    mgVideoChat.prototype.onRoomOptions = function(){
        ...
        if(this.roomOptions.group || this.roomOptions.roulette){
            ...
            //get media stream callback
            var onMediaStream = function(success){
                rtc.mediaReady();
            ...
            //create stream
            rtc.createStream(null, this.getMediaOptions({audio:true, video:true})
                    ...
                    fileHelper.send(file, channel, {
                        onFileProgress: function(param){
                            ...
                            if(self.files[param.fileId]){
                                self.files[param.fileId].progress = param;
        ...
        rtc.on('file_sdp_offer', function(data) {
            ...
            rtc.fileSdpAnswer(data.connectionId, data.fileDesc, {
                channelOnMessage: function(param){                    
                    ...
                    self.debug('channelOnMessage connId: '  + data.connectionId + ', order: ' + data.order);
                    fileHelper.receive(data, {
                        onFileProgress: function(param){
                            self.debug('onFileProgress ' + fileId + ' connId: '  + data.connectionId);
                            self.debug(param);
                            if(self.files[param.fileId]){
                        ...
                        onFileReceived: function(name, param){
                            self.debug('onFileReceived file: ' + name + ', file id: ' + fileId + ' connId: '  + data.connectionId);
                            self.debug(param);
                            delete self.files[param.fileId];
                            self.renderFiles();
                        }
    ...
    mgVideoChat.prototype.getConnectionElement = function(connectionId, callback){
        ...
        this.loadTplByName('tplConnection', function(connTpl){
            ...
            if (self.roomOptions.group && connection.rstream){
                $content.find('video').attr("src",window.URL.createObjectURL(connection.rstream));
            }
            if (self.roomOptions.group){
                //DOM
    ...
    mgVideoChat.prototype.renderFileProgress = function(fileId){
        ...
        var progress = 0;
        if(file.progress){
            progress = (file.progress.transfered / file.progress.length) * 100;
        }
    ...
    mgVideoChat.prototype.setChat = function(chatId){
        ...
        if(chatId > 0){
            this.$chatPanel.find('.panel-title').text(this._('Chat con {username}',['{username}'],[rtc.connections[chatId]['data']['userData']['name']]));
        }
        else{
    ...
    mgVideoChat.prototype.parseChatMessageText = function(messageText){
        ...
        return nl2br(linkify(htmlspecialchars(messageText)),false);
    }
...

{
    ...
    rtc.checkCompatibility = function (errors){
        rtc.compatible = true;
        if(!window.WebSocket){
    ...
    rtc.fileGetPeerConnection = function(connectionId, options, sending) {
        try{
            ...
            if(!rtc.firefox){
                var params = {optional: [{RtpDataChannels: true}]};
            }
            else{
                var params = {optional: []};
            }
        ...
        var createSendChannel = function(){
            ...
            var chOptions = (rtc.firefox)? {} : {reliable: false};
            rtc.connections[connectionId].sendChannel = rtc.connections[connectionId].dpc.createDataChannel("sendDataChannel" + connectionId, chOptions);
            if (rtc.firefox){
                rtc.connections[connectionId].sendChannel.binaryType = 'blob';
            }
        ...
        pc.ondatachannel = function(event) {
            if(!sending){
                ...
                if (rtc.firefox){
                    rtc.connections[connectionId].receiveChannel.binaryType = 'blob';
                }
